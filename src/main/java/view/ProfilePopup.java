...
public class ProfilePopup {
    ...
    public static JPanel createProfilePanel(String image, String fullName, String location, String gender,
                                            Date dateOfBirth, String bio, List<String> tags) {
        // Define the width for the 4:5 ratio
        int width = 310;
        int height = (width * 5) / 4;
        ...
        JPanel mainPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                int width = getWidth();
                int height = getHeight();
                GradientPaint gradient = new GradientPaint(0, 0, new Color(255, 192, 203), 0, height, new Color(255, 105, 180)); // Example colors
                g2d.setPaint(gradient);
                g2d.fillRect(0, 0, width, height);
            }
        };
        ...
        // Image panel
        ImagePanel imagePanel = new ImagePanel(image); // Assuming ImagePanel handles image rendering
        imagePanel.setPreferredSize(new Dimension(330, 200)); // Set image panel size
        mainPanel.add(imagePanel); // Add the image panel to the main panel
        ...
        tagPanel.setBackground(new Color(255, 245, 247)); // Background color for the tag panel

        // Adding tags as labels with styles
        for (String tag : tags) {
            JLabel tagLabel = new JLabel(tag);
            tagLabel.setOpaque(true);
            tagLabel.setBackground(new Color(173, 216, 230)); // Example color for tags
            tagLabel.setForeground(Color.WHITE);
            tagLabel.setFont(new Font("Arial", Font.PLAIN, 12));
            tagLabel.setBorder(BorderFactory.createEmptyBorder(5, 10, 5, 10)); // Padding inside the label
            tagPanel.add(tagLabel); // Add each label to the tag panel
        }

        mainPanel.add(tagPanel);

        return mainPanel; // Return the profile panel
    }
...
//     Main function to test the profile dialog
    public static void main(String[] args) {
        ...
        createProfilePanel(image, fullName, location, gender, dateOfBirth, bio, tags);
        JPanel profilePanel = createProfilePanel(image, fullName, location, gender, dateOfBirth, bio, tags);
